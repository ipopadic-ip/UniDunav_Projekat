...
public class KatedraService {
	...
	public Katedra update(Long id, Katedra noviPodaci) {
        return katedraRepository.findById(id).map(katedra -> {
        	katedra.setNaziv(noviPodaci.getNaziv());
        	katedra.setPredmeti(noviPodaci.getPredmeti());
        	katedra.setOpis(noviPodaci.getOpis());
        	katedra.setTipoviStudija(noviPodaci.getTipoviStudija());
